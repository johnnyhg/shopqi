	...
	$(function() {
		...
		var u = navigator.userAgent.toLowerCase(),
			v = (u.match( /.+?(?:rv|it|ra|ie)[\/: ]([\d.]+)/ ) || [0,'0'])[1],
                        // change '.jstree li' to '.jstree ul li', avoid margin-left conflict with formtastic css. by saberma
			css_string = '' + 
				'.jstree ul, .jstree li { display:block; margin:0 0 0 0; padding:0 0 0 0; list-style-type:none; } ' + 
				'.jstree ul li { display:block; min-height:18px; line-height:18px; white-space:nowrap; margin-left:18px; } ' + 
				...
				var rtl = this._get_settings().core.rtl,
					...
					$("<input>", { 
						...
						"css" : {
							...
							"position" : "absolute",
                                                        // without icon, we don't need w2. by saberma
							//"left"  : (rtl ? "auto" : (w1 + w2 + 4) + "px"),
							"left"  : (rtl ? "auto" : (w1 + 4) + "px"),
		...
		if($.jstree._themes === false) {
			$("script").each(function () { 
				if(this.src.toString().match(/jstree[^\/]*?\.js(\?.*)?$/)) { 
					$.jstree._themes = this.src.toString().replace(/jstree[^\/]*?\.js(\?.*)?$/, "") + 'themes/'; 
					return false; 
				}
			});
		}
